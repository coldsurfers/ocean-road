...
:::tip
`RouteLoading`은 앱 레이아웃(layout.tsx)에 배치하고, 각 페이지 컴포넌트를 `children`으로 감쌉니다.
:::
...
기본적으로 `RouteLoading`은 마운트 시 로딩 상태를 자동으로 초기화합니다. 특정 데이터 fetching이 끝난 후 로딩을 해제하려면 `deps`를 사용합니다.
